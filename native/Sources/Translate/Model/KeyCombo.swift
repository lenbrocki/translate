...
/// A global keyboard shortcut: a physical key plus its modifiers.
///
/// The key code is positional, so the same combination stays on the same
/// physical key whatever layout the user types in.
struct KeyCombo: Codable, Equatable, Sendable {
    var keyCode: UInt16
    /// Raw value of the device-independent part of `NSEvent.ModifierFlags`.
    var modifiers: UInt
    ...
    /// Carbon needs its own modifier bits; `RegisterEventHotKey` takes these.
    ...
    /// At least one non-shift modifier, or the combination would swallow
    /// ordinary typing system-wide.
    var isValid: Bool {
        !modifierFlags.intersection([.command, .option, .control]).isEmpty
    }

    /// `⌘⇧T`, in the order macOS writes modifiers in menus.
    ...
    static func keyLabel(for keyCode: UInt16) -> String {
        ...
        return characterKeys[Int(keyCode)] ?? "Key \(keyCode)"
    }
